...
# -------------------------
# Markov with punctuation and grammar preferences
# -------------------------
class Markov:
    def __init__(self):
        self.map = {}
    ...
    def generate(self, seed=None, max_words=40, capitalize_if=True):
        if seed:
            ...
            if len(toks) >= 2:
                ...
                for _ in range(max_words):
                    ranked = self._rank_candidates(key)
                    if not ranked:
                        candidates = {}
                        ...
                        ranked = self._rank_candidates(temp_key)
        ...
        for _ in range(max_words-2):
            ranked = self._rank_candidates((out[-2], out[-1]))
            if not ranked:
                break
            nxt = ranked[0][0]
            out.append(nxt)
